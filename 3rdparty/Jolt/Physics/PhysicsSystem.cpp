...
void PhysicsSystem::ProcessBodyPair(ContactAllocator &ioContactAllocator, const BodyPair &inBodyPair)
{
	...
	if (!pair_handled)
	{
			...
			&& body1->GetUseManifoldReductionWithBody(*body2))	// Check body flag
		{
			...
			CollisionDispatch::Instance()->sCollideShapeVsShape(body1->GetShape(), body2->GetShape(), Vec3::sReplicate(1.0f), Vec3::sReplicate(1.0f), transform1, transform2, part1, part2, settings, collector);
